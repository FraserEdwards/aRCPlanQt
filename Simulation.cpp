...
//		plot.handler(solution);
		...
		controlCode=interface.input("FINISHED.  Enter 0 to end, any other integer to run another case: ");

		if (controlCode == 0)
		{
			// Check for user input
			controlCode=interface.input("Are you **SURE** you want to exit?.  Enter 0 again to end, any other integer to run another case: ");

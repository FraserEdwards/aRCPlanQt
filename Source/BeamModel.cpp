...
void BeamModel::reset(const Parameters parameters, const Backfill backfill, Creep creep)
{
    ...
    pressurefactor = pow(parameters.sdr / sdrminus1, 4) / adotclfactor;
    residualpressure = 2.0 / 3.0 * parameters.dynamic_modulus / pow(parameters.sdr / sdrminus1, 3) * (1.0 - parameters.diameter_creep_ratio);
...
void BeamModel::opening(Parameters parameters, Creep creep)
{
    ...
    {	// then recalculate and print the numerical solution
        ...
        FDprofile final(alpha, m, zetabackfilled, pressurefactor, residualpressure, parameters.elements_in_l, node_at_closure);
        //            FDprofile* ptr=&final;
        final.fprofile();

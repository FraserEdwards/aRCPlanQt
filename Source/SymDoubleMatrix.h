...
//  but despite its name it is NOT restricted to symmetrical matrices
{
	public:
	    ...
	    SymDoubleMatrix(const SymDoubleMatrix& a);			// Create a copy of the existing matrix a
		double getElement(short i, short j);				// Return value of element (i, j), j>=i
		void setElement(short i, short j, double x);		// Assign value x to element (i, j), j>=i
		void incrementElement(short i, short j, double dx);	// Add dx to element (i, j), j>=i
	    SymDoubleMatrix& operator=(const double xij);		// Assignment to scalar by '='
	...
	private:
	    ...
	    short* pointer2indices;								// Pointer to matrix records row permutations effected by partial pivoting
};
